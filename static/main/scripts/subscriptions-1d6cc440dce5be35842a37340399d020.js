...
main.addEventListener('submit', async e =>
{
    if (e.target.matches('form[id^="form-anime-"]'))
    {
        ...
        if (cacheManifest)
        {
            url = '/static/' + cacheManifest['general/media/emotes/pepehands.png'];  
        }
        else
        {
            url = '/static/general/media/emotes/pepehands.png'
        }

        updateSub(
            e.target,
            submitBtn => rmTitle(submitBtn, false),
            [url, 'pepehands', 'Brak subskrypcji']
        ).catch(console.log);                
    }
});
